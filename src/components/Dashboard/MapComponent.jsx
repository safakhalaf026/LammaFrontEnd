...
const MapComponent = () => {
    ...
    // only fetching the markers for the displayed location 
    ...
    useEffect(() => {
        // if no there is no map, do nothing  
        if (!mapRef.current || !serviceData) return;
        // to prevent duplicate markers
        markersRef.current.forEach(marker => marker.remove());
        markersRef.current = [];

        // placing markers by looping through each service
        ...
        services.forEach((service) => {
            ...
            const marker = new mapboxgl.Marker({ color: 'red' })
                .setLngLat([longitude, latitude]) // coordianntes
                .setPopup(popup)
    ...
    }, [serviceData]); // only when data changes
